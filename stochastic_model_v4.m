...
function [mh1,Time] = stochastic_model_v4(param_set)
%nrun=1;%30;
...
mh1 = [mh1(1,:),mh1(2,:)];
sync_score = corr(mh1(1,:),mh1(2,:),'Type','Pearson');
...
% xlabel('Time')
% ylabel('#mRNA')
% saveas(gcf,strcat(['Run_v4', num2str(run)]),'jpg');
% close(gcf);
% Smooth version of the curve using Moving Average with k=5, which means 4 neighbors
% figure
% plot(Time, mh1v_c1,'b')
% hold on
% plot(Time, Smooth_mh1v_c1,'k')
% saveas(gcf,strcat('Run_v4_Smooth', num2str(run)),'jpg');
% %close(gcf)
% 
end

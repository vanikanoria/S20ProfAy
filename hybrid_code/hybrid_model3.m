function [Y] = hybrid_model3 (x) % x: parameter set of size 44
% Hybrid model for a single cell system
rng(1); %seeds the random number generator
...
function [value,isterminal,direction] = events(t,y)
    value      = y(num_states+1:end); 
    isterminal = ones(size(stochreactions,2),1);
    direction  = ones(size(stochreactions,2),1);
end
...
function s = start_delayed_reaction(r,s)
    ...
    function [y,s] = perform_stochastic_reaction(y,s,r)
   ...
   % update species levels accordingly and draw new random number
        y = max(y(1:num_states)+R(:,r),0); 
    end 
    end
    
    function [y,s] = end_delayed_reaction(y,s,r)
        s{r}=s{r}(2:Td(r));
        Td(r) = Td(r)-1;
        y = max(y(1:num_states)+R(:,r),0);
    end
    ...
    function ydot = update_ode(t,y)   
        ...
        ydot
        %last part: RHS for each stochastic reaction
        % 24 different differential equations --> we pick the event from
        % the reaction that fires first. Earlier we had to pick a reaction
...
stocnum = size(stochreactions,2);
global logrand;
logrand=log(rand(stocnum,1));
y0 = [ones(num_states,1); logrand];
...
while t < Tend
    ...
    [tsol,ysol,te,ye,ie] = ode45(@update_ode,[t t+tau_n],[y(1:num_states); logrand],options3);
    ...
    if ~isempty(ie)
       ...
       t=te;
       r=stochreactions(ie);
       [y,s] = perform_stochastic_reaction(y,s,r);
       logrand=ye(num_states+1:end)';
       logrand(ie)=log(rand);   
            if length(ie) > 1 || ie>12
             error('ODEsolution:Events','\nMore than one event detected.');
            end
    else
        y=ysol(end,1:num_states)';
        t=tsol(end);
        logrand=ysol(end,num_states+1:end)';
        if reaction==7
            reaction=8;
        end
